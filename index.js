...
function isPrimeNumber(num) {
  ...
  for (let i = 2; i < num; i++) {
    if (num % i === 0) {
      return false; // Якщо число num ділиться націло на i, воно не є простим
    }
  }

  return true; // Якщо під час циклу не знайдено дільників, число є простим
}
...
function checkDataType(value) {
  const valueType = typeof value; // Отримуємо тип значення value за допомогою оператора typeof
  console.log(`Значення "${value}" має тип "${valueType}".`); // Виводимо у консоль повідомлення про тип значення
  return valueType; // Повертаємо тип значення value
}
...
function getUserStatus(age) {
  if (age < 18) {
    return "Неповнолітній"; // Якщо age менше 18, то користувач є "Неповнолітнім"
  } else if (age >= 18 && age < 65) {
    return "Дорослий"; // Якщо age більше або дорівнює 18 і менше 65, то користувач є "Дорослим"
  } else {
    return "Пенсіонер"; // В іншому випадку (якщо age більше або дорівнює 65), то користувач є "Пенсіонером"
  }
...
function getVariableType(variable) {
  const type = typeof variable; // Отримуємо тип змінної variable за допомогою оператора typeof
  const message =
    type === "number"
      ? "Це число"
      : type === "string"
      ...
      : type === "boolean"
      ? "Це булеве значення"
      : "Це інший тип"; // Встановлюємо відповідне повідомлення залежно від отриманого типу
  return message; // Повертаємо встановлене повідомлення як результат функції
...
function factorial(n) {
  if (n === 0 || n === 1) {
    return 1; // Якщо n дорівнює 0 або 1, повертаємо 1, оскільки факторіал 0 і 1 завжди дорівнює 1.
  } else {
    return n * factorial(n - 1); // Якщо n більше за 1, ми множимо n на результат виклику тієї самої функції з n-1.
  }
}
...
function makeAdder(x) {
  return function (y) {
    return x + y; // Вертаємо суму `x` та `y`
  };
}
// Створюємо змінну getAdder, якій присвоюємо результат виконання функції makeAdder з аргументом 5
...
function divide(x) {
  return function (y) {
    return x / y; // Ділимо `x` на `y` і повертаємо результат
  };
}

// Створюємо змінну getDivider, якій присвоюємо результат виконання функції divide з аргументом 2
...
function compose(func1, func2) {
  return function (n) {
    return func2(func1(n)); // Композиція функцій: спочатку застосовуємо func1, потім func2
  };
}
